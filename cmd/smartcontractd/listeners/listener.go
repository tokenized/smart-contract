...
// handler.
func (server *Server) Handle(ctx context.Context, msgType int, msgValue interface{}) (bool, error) {
	...
	case handlers.ListenerMsgTx:
		...
		if !ok {
			return false, errors.New("Could not assert msgValue as wire.MsgTx")
		}
	...
	case handlers.ListenerMsgTxConfirm:
		...
		if !ok {
			return false, errors.New("Could not assert msgValue as chainhash.Hash")
		}
	...
	case handlers.ListenerMsgBlock:
		...
		if !ok {
			return false, errors.New("Could not assert msgValue as BlockMessage")
		}
	...
	case handlers.ListenerMsgInSync:
		...
		for _, pendingTx := range pending {
			...
			if err := server.sendTx(ctx, pendingTx); err != nil {
				return false, err // TODO Probably a fatal error
			}
		}
	}
