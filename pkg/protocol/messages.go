...
// Addition Adds a User's public address to a global distributed whitelist.
// Entities (eg. Issuer) can filter by the public address of known and
// trusted entities (eg. KYC Databases such as coinbase) and therefore are
// able to create sublists - or subsets - of the main global whitelist.
...
// AssetDefinition (General Template) [This action is used by the issuer to
// define the properties/characteristics of the Asset (token) that it wants
// to create.]
...
// AssetModification (General Template) [Token Dilutions, Call
// Backs/Revocations, burning etc.]. Any field can be amended except for
// the Asset Revision field (incremental counter based on the previous
// Asset Creation Txn) and the Action Prefix. Asset Types specific payloads
// are locked to the Asset Type. Asset Type specific payload amendments
// must be done as a protocol Version upgrade. Authorization flags can
// restrict some fields or all fields from being amended. Some amendments
// require a Token Owner vote for the smart contract to permit.
...
func (m Message) Bytes() ([]byte, error) {
	...
	if err := m.write(buf, m.pad(m.Message, 211)); err != nil {
		return nil, err
	}
...
// Referendum Issuer instructs the Contract to Initiate a Token Owner Vote.
// Usually used for contract amendments, organizational governance, etc.
...
// and Ballot Cast actions that do not comply with the Contract. If money
// is to be returned to a User then it is used in lieu of the Settlement
// Action to properly account for token balances. All Issuer/User Actions
// must be responded to by the Contract with an Action. The only exception
// to this rule is when there is not enough fees in the first Action for
// the Contract response action to remain revenue neutral. If not enough
// fees are attached to pay for the Contract response then the Contract
// will not respond. For example
...
func (m Rejection) Bytes() ([]byte, error) {
	...
	if err := m.write(buf, m.pad(m.Message, 145)); err != nil {
		return nil, err
	}
...
// Send A Token Owner Sends a Token to a Receiver. The Send Action requires
// no sign-off by the Token Receiving Party and does not provide any
// on-chain consideration to the Token Sending Party. Can be used for User
// Revocation (remove tokens from wallet by sending back to Issuer). Can be
// used for redeeming a ticket, however, it is probably better for most
// ticket use cases to use the exchange action for ticket redemption.
type Send struct {
	BaseMessage
	Header       []byte
	ProtocolID   uint32
...
// Swap Two parties want to swap a token (Atomic Swap) directly for another
// token. No BCH is used in the txn other than for paying the necessary
...
// Vote A vote is created by the Contract in response to a valid Referendum
// (Issuer) or Initiative (User) Action. Votes can be made by Token Owners.
type Vote struct {
	BaseMessage
	Header               []byte
	ProtocolID           uint32
